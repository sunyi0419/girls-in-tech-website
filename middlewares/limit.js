    ...
    return function (req, res, next) {
      ...
      cache.get(key, function (err, count) {
        ...
        if (count < limitCount) {
          ...
          res.status(403);
          if (options.showJson) {
            res.send({success: false, error_msg: 'Frequency Limit: this action may not exceed ' + limitCount + ' times per day.'});
          } else {
            res.render('notify/notify', { error: 'Frequency Limit: this action may not exceed ' + limitCount + ' times per day.'});
          }
        }
      });
    };
