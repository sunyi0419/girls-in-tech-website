...
exports.getFullTopic = function (id, callback) {
  ...
  Topic.findOne({_id: id, deleted: false}, proxy.done(function (topic) {
    if (!topic) {
      proxy.unbind();
      return callback(null, 'This topic is not exist or has been deleted');
    }
    ...
    User.getUserById(topic.author_id, proxy.done(function (author) {
      if (!author) {
        proxy.unbind();
        return callback(null, 'The author of this topic is lost');
      }
...
exports.reduceCount = function (id, callback) {
  Topic.findOne({_id: id}, function (err, topic) {
    ...
    if (!topic) {
      return callback(new Error('This topic is not exist'));
    }
    topic.reply_count -= 1;
