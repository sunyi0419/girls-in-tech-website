...
exports.add = function (req, res, next) {
  ...
  if (str === '') {
    return res.renderError('Reply cannot be empty', 422);
  }
...
exports.showEdit = function (req, res, next) {
  ...
  Reply.getReplyById(reply_id, function (err, reply) {
    if (!reply) {
      return res.render404('This reply is not exist or has been deleted');
    }
    if (req.session.user._id.equals(reply.author_id) || req.session.user.is_admin) {
      ...
      return res.renderError('Sorry, you have no permission to edit this reply', 403);
    }
  });
};
/*
  ...
  Reply.getReplyById(reply_id, function (err, reply) {
    ...
    if (String(reply.author_id) === req.session.user._id.toString() || req.session.user.is_admin) {
      ...
      if (content.trim().length > 0) {
        ...
        return res.renderError('Reply too short', 400);
      }
    } else {
      return res.renderError('Sorry, you have no permission to edit this reply', 403);
    }
...
exports.up = function (req, res, next) {
  ...
  Reply.getReplyById(replyId, function (err, reply) {
    ...
    if (reply.author_id.equals(userId) && !config.debug) {
      ...
      res.send({
        success: false,
        message: 'Please do not endorse yourself',
      });
    } else {
      var action;
      reply.ups = reply.ups || [];
