...
exports.index = function (req, res, next) {
  ...
  User.getUserByLoginName(user_name, function (err, user) {
    ...
    if (!user) {
      res.render404('No user in this name');
      return;
    }
    ...
    var render = function (recent_topics, recent_replies) {
      ...
      res.render('user/index', {
        ...
        token: token,
        pageTitle: util.format('@%s\'s profile', user.loginname),
      });
    };
...
exports.showSetting = function (req, res, next) {
  User.getUserById(req.session.user._id, function (err, user) {
    ...
    if (req.query.save === 'success') {
      user.success = 'Successfully Save';
    }
    user.error = null;
...
exports.setting = function (req, res, next) {
  ...
  if (action === 'change_password') {
    ...
    if (!old_pass || !new_pass) {
      return res.send('old password or new password cannot be empty');
    }

    User.getUserById(req.session.user._id, ep.done(function (user) {
      tools.bcompare(old_pass, user.pass, ep.done(function (bool) {
        if (!bool) {
          return showMessage('Password wrong', user);
        }
        ...
        tools.bhash(new_pass, ep.done(function (passhash) {
          ...
          user.save(function (err) {
            ...
            return showMessage('Password modified', user, true);
